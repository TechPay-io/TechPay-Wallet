<template>
    <div class="view-defi-manage-collateral">
        <h1 class="with-back-btn"><f-back-button :route-name="backButtonRoute" /> Lock/Unlock {{ wtpcTokenSymbol }}</h1>

        <h2 class="perex">
            Lock {{ wtpcTokenSymbol }} to increase the collateral ratio and mint tUSD, unlock
            {{ wtpcTokenSymbol }} after you repaid tUSD.
        </h2>
        ...
        <div class="grid">
            <div>
                <div class="df-data-item smaller">
                    ...
                    <div class="value"><f-token-value :token="wtpcToken" :value="availableTPC" /></div>
                </div>
                <div class="df-data-item smaller">
            ...
            <div class="defi-price-input-col align-center">
                <div class="defi-price-input">
                    ...
                    <div class="token-label">{{ wtpcTokenSymbol }}</div>
                    <div class="collateral-info">
                        <div class="collateral-info-label">Unlock {{ wtpcTokenSymbol }}</div>
                        <icon data="@/assets/svg/angle-double-left.svg" width="66" height="66" aria-hidden="true" />
                    </div>
                    <div class="collateral-info increase">
                        <div class="collateral-info-label">Lock {{ wtpcTokenSymbol }}</div>
            ...
            <f-message v-else-if="increasedCollateral > 0" type="info" role="alert" class="big">
                ...
                <span class="inc-desc-collateral">
                    <f-token-value :token="wtpcToken" :value="increasedCollateral" no-currency /> {{ wtpcTokenSymbol }}
                </span>
                to your collateral
            </f-message>
            <f-message v-else-if="decreasedCollateral > 0" type="info" role="alert" class="big">
                You’re removing
                <span class="inc-desc-collateral">
                    <f-token-value :token="wtpcToken" :value="decreasedCollateral" no-currency /> {{ wtpcTokenSymbol }}
                </span>
...
*/
        ...
        _maxCollateral() {
            let max = this.collateral + this.availableTPC - 1;
    ...
    methods: {
        ...
        formatInputValue(_value) {
            return parseFloat(_value).toFixed(this.$defi.getTokenDecimals(this.wtpcToken));
        },

        updateMessage() {
            if (this.availableTPC <= 0.01) {
                this.message = `Deposit more ${this.wtpcTokenSymbol} to increase your collateral`;
            } else {
                this.message = '';
            }
