        ...
        <template v-if="success">
            ...
            <ul class="no-markers" style="margin-bottom: 16px;">
                <li v-for="result in tsData.result" :key="result.uuid">
                    <b>
                        {{ result.amount }}
                        {{ $bnb.getTPCCurrencyByDirection(receive ? tsData.direction : 'PhotonToPhoton') }}
                    </b>
                    from
                    <b class="break-word">{{
                        receive ? tsData[$bnb.getAddressKeyByDirection(tsData.direction)] : tsData.from_photon_address
                    }}</b>
                </li>
            </ul>
            ...
            <p>
                ...
                <b>
                    ...
                    {{ $bnb.getTPCCurrencyByDirection(receive ? 'PhotonToPhoton' : tsData.direction) }}
                </b>
                in your address
                <b> {{ tsData.address || tsData.result[0].photon_address }}</b>
            </p>
